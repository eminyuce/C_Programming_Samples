...
#define S 100 /* number of rows in matrix */
...
void readVector();
void writeVector();
void GaussianElimination(int,int);
...
GaussianElimination(taskid,numtasks);
...
void GaussianElimination(int taskid, int pSize){
int i, j, k, n,mod;
n=S;
...
* FILE: mm.c
* DESCRIPTION: 
* A MPI program that implements Back Subsutition.
* AUTHOR: Emin Yüce 
******************************************************************************/
...
/*Process 0 send matrix data to the slaves */
if (taskid == MASTER) {	
//Read a matrix from an input file called "matrix.out" 
readMatrix();
//And a vector from an input file called "vector.out"
readVector();
...
destination = i % numtasks;
if(destination != MASTER){
MPI_Send(U[i], S, MPI_FLOAT, destination, 0, MPI_COMM_WORLD);
}
}	
...
source = i % numtasks;
if(source == taskid){
MPI_Recv(U[i], S, MPI_FLOAT, MASTER, 0, MPI_COMM_WORLD, &status);
}
}

}
//The vector y is not striped ampng the processes;
MPI_Bcast(y, S, MPI_FLOAT, MASTER , MPI_COMM_WORLD);
Back_Subsutition(taskid,numtasks);
...
if(destination == taskid){
if(destination != MASTER){
MPI_Send(&y[i], 1, MPI_FLOAT, MASTER, 3, MPI_COMM_WORLD);
}
}else if(taskid == MASTER){
MPI_Recv(&y[i], 1, MPI_FLOAT,destination, 3, MPI_COMM_WORLD, &status);
}
}
//MASTER process 0 writes result matrix and result vector in files called "result.out"
if (taskid == MASTER) {
last = MPI_Wtime();
printf("Elapsed Time of Computation is: %f\n", last - first);
writeResults();
...
void Back_Subsutition(int taskid,int pSize){
int i,j,k,n,mod;
n=S;
for(k=n; k>=0; k--){
mod = k % pSize;
...
MPI_Bcast(&x[k], 1, MPI_FLOAT, mod , MPI_COMM_WORLD);
